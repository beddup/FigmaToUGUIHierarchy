...
Figma text node bounds often describe only the current demo copy's tight visual bounding box. Do not blindly use the original text node `bounds` as the Unity runtime text display area.
Infer a practical runtime text rectangle that can support localization and longer strings while preserving the screenshot's design intent.

**Primary invariants:**
- `text_rect` is the design-intended runtime display area for this text. It is not the current Figma layout or raw Figma data. Infer it from the screenshot, surrounding design elements, and visible design intent, rather than copied from Figma data.
- `text_rect` plus `text_alignment` must reproduce the original Figma text's rendered visual position. The source text must not visibly drift when rendered inside the inferred rectangle.
- `text_rect` uses the same global coordinate space and units as the simplified Figma `flatNodes[].bounds` field.

### `text_rect`

`text_rect` is the inferred runtime text display rectangle. Use integer values.
...
Inference guidance:
- Choose alignment together with `text_rect`; they are one inferred runtime layout decision, not two independent fields.
- Alignment should express how this text is meant to sit inside the inferred text display slot according to the same visible design intent.
- Do not choose alignment from metadata, names, parent type, or any single bounds rectangle alone; when they conflict with the inferred text display slot, prefer the runtime intent expressed by the visible design.
- After choosing `text_alignment`, re-check the primary invariants: rendering the original Figma text inside the inferred `text_rect` with this alignment must still match the original Figma visual position. If it does not, revise the rectangle or alignment before saving.
